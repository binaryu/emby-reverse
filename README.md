...
> 安装脚本优先从 GitHub Releases 下载二进制。若仓库还没有 Release 资产（只有 tag），会**自动回退到源码编译**：本机 Go ≥ 1.21 时用 `GOTOOLCHAIN=auto`；更旧或没有 Go 时会临时下载便携 Go 1.25.x 再编译。也可打 tag 触发 Actions 发版：`git tag v1.x.x && git push origin v1.x.x`。
